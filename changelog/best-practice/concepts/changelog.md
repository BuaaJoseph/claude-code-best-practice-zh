...
| 12 | LOW | Changed 描述 | Update Memory description to mention auto memory | ✅ COMPLETE (description and location updated) |
| 13 | LOW | Changed Location | Update MCP Servers location to include `.mcp.json` | ✅ COMPLETE (location updated to include .mcp.json) |
| 14 | LOW | Missing Badge | Add Implemented badge to 钩子s row | ✅ COMPLETE (Implemented badge added linking to .claude/hooks/) |
...
| 4 | MED | Merged Concept | Fold Permissions, Model Config, Output Styles, Sandboxing, Keybindings, Status Line, Fast Mode into 设置 row as inline links | ✅ COMPLETE (7 concepts folded with doc links) |
...
| # | Priority | 类型 | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Broken URL | Fix 命令 URL from `/slash-commands` to `/skills` in CONCEPTS table (line 24) — `/slash-commands` serves 技能 page content; docs say "commands merged into skills" | ❌ INVALID (URL still resolves; user chose to keep as-is) |
| 2 | HIGH | Broken URL | Fix 命令 URL from `/slash-commands` to `/skills` in TIPS section (line 108) — same stale URL | ❌ INVALID (URL still resolves; user chose to keep as-is) |
...
| 4 | MED | Missing Inline Link | Add Costs (`/costs`) as inline link to 设置 row — covers /usage, billing, pay-as-you-go | ❌ INVALID (user chose to skip) |
...
| # | Priority | 类型 | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Broken URL | Fix 命令 URL from `/slash-commands` to `/skills` in CONCEPTS table (line 24) — `/slash-commands` redirects to `/skills` page | ❌ INVALID (RECURRING from 2026-03-10; URL still resolves; user chose to keep as-is) |
...
| # | Priority | 类型 | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Stale URL | 命令 URL `/slash-commands` serves 技能 page — docs say "commands merged into skills" | ❌ INVALID (RECURRING from 2026-03-10; URL still resolves; user chose to keep as-is) |
...
| 3 | LOW | Naming | "Sub-Agents" in README vs "subagents" (one word) in official docs — cosmetic inconsistency | ✅ COMPLETE (renamed to "子代理" in CONCEPTS table) |
...
|---|----------|------|--------|--------|
| 1 | HIGH | Stale URL | 命令 URL `/slash-commands` serves 技能 page — docs say "commands merged into skills" | ❌ INVALID (RECURRING from 2026-03-10; URL still resolves; user chose to keep as-is) |
| 2 | HIGH | Changed 描述 | 钩子s description says "Deterministic scripts" but hooks now include 4 types: command, HTTP, prompt, and agent — only command hooks are deterministic | ✅ COMPLETE (updated to "User-defined handlers (scripts, HTTP, prompts, agents)" in CONCEPTS table) |
...
| 4 | MED | Changed URL | 钩子s docs now split into Guide (`/hooks-guide`) and Reference (`/hooks`) — CONCEPTS links only to Reference | ✅ COMPLETE (Guide link added as inline link in 钩子s row description) |
...
| 8 | LOW | Verification | All CONCEPTS descriptions checked against official docs | ✅ COMPLETE (钩子s description drift detected — see #2) |
...
| 2 | HIGH | Stale URL | 命令 URL `/slash-commands` serves 技能 page — docs say "commands merged into skills" | ❌ INVALID (RECURRING from 2026-03-10; URL still resolves; user chose to keep as-is) |
...
| 9 | LOW | Verification | Bundled 技能 anchor `#bundled-skills` confirmed on /skills page | ✅ COMPLETE (section heading exists) |
...
| 8 | LOW | Verification | Bundled 技能 anchor `#bundled-skills` confirmed on /skills page | ✅ COMPLETE (section heading exists) |
...
| 3 | LOW | Changed 描述 | Plugins description missing LSP servers — official docs list `.lsp.json` as plugin component | ✅ COMPLETE (added "and LSP servers" to Plugins description) |
